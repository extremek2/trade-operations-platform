...
BEGIN READ ONLY;
SELECT version,success FROM flyway_schema_history WHERE version IN ('1','2','3') ORDER BY installed_rank;
...
DO $$ BEGIN
  IF (SELECT count(*) FROM flyway_schema_history WHERE version IN ('1','2','3') AND success)<>3 THEN
    RAISE EXCEPTION 'Expected application migrations V1-V3';
  END IF;
END $$;
COMMIT;
