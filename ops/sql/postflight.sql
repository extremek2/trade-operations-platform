...
BEGIN READ ONLY;
SELECT version,success FROM flyway_schema_history WHERE version IN ('1','2') ORDER BY installed_rank;
...
DO $$ BEGIN
  IF (SELECT count(*) FROM flyway_schema_history WHERE version IN ('1','2') AND success)<>2 THEN
    RAISE EXCEPTION 'Expected application baseline migrations V1-V2';
  END IF;
END $$;
COMMIT;
