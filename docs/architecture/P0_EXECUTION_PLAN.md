...
1. 최소 `CostScenario` Aggregate와 불변 계산 입력 스냅샷을 정의한다.
2. 환율·상품가·운임·세금·국내비용을 `BigDecimal`로 계산하는 순수 도메인 규칙을 구현한다.
3. 반올림·배부·nullable·값 출처 규칙을 도메인 테스트로 먼저 고정한다.
4. 계산 API를 연결한 뒤 입력·비교 화면을 추가한다.
